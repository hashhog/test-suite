#!/usr/bin/env bash
#
# run-scanblocks-regression.sh — scanblocks indexing/UTXO Core-parity differential.
#
# Runs each per-impl test in scanblocks/<impl>_scanblocks.sh. Each test is self-contained:
...
# share a byte-identical tip, then runs `scanblocks start "addr(<addr>)"` on BOTH and
# diffs. Core ref: bitcoin-core/src/rpc/blockchain.cpp (scanblocks). EXACT shape:
...
#
#   Usage:  run-scanblocks-regression.sh
#           SCANBLOCKS_IMPLS="rustoshi nimrod" run-scanblocks-regression.sh
#
# HEAVY: a regtest impl node + a Core oracle per impl, SEQUENTIALLY. exit 0 = no regression.
DIR="$(cd "$(dirname "$0")" && pwd)"
SCANB="$DIR/scanblocks"
...
IMPLS="${SCANBLOCKS_IMPLS:-rustoshi nimrod ouroboros blockbrew hotbuns camlcoin beamchain clearbit lunarblock haskoin}"
LOGDIR="${SCANBLOCKS_LOGDIR:-/tmp/scanblocks-regression}"
mkdir -p "$LOGDIR"
...
for impl in $IMPLS; do
  script="$SCANB/${impl}_scanblocks.sh"
  if [ ! -x "$script" ] && [ ! -f "$script" ]; then
    echo "  SKIP  $impl (no scanblocks test yet: $script)"; SKIP=$((SKIP+1)); continue
  fi
  log="$LOGDIR/${impl}.log"
  tmpout="$(mktemp)"
