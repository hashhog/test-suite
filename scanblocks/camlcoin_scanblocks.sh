#!/usr/bin/env bash
#
# camlcoin_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#
# Core ref: bitcoin-core/src/rpc/blockchain.cpp scanblocks (action start/status/abort).
...
#   into camlcoin via submitblock. After replay both nodes hold the
...
#     which BIP-158 includes from undo data, AND the spend block's coinbase
#     also pays ADDR).
#
# WHAT MUST HOLD (per the strict fan-out contract — EXACTLY mirrors
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS camlcoin: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS camlcoin: FAIL <short reason>
...
log() { echo "[scanblocks:camlcoin] $*" >&2; }
...
# ── Summary emitters. ─────────────────────────────────────────────────────
pass() { echo "SCANBLOCKS camlcoin: PASS scan=$1 shape=$2 range=$3 errors=$4"; exit 0; }
fail() { echo "SCANBLOCKS camlcoin: FAIL $*"; exit 1; }
skip() { echo "SCANBLOCKS camlcoin: SKIP $*"; exit 0; }
...
# Retries up to 3x on a transient EMPTY response: an empty body carries neither
# "result" nor "error", so a genuine JSON-RPC error response is returned
...
# NOTE: camlcoin is launched AFTER Core finishes building the chain (section 6),
# not here. Launching it first and letting it sit idle through Core's
# chain-build phase makes the sandbox watchdog SIGKILL the idle camlcoin
# process. The proven getblockfilter harness builds the Core chain first too.

# ── 5. Core mines a chain that INCLUDES A SPEND. ──────────────────────────
# Determinism (setmocktime) is NOT needed here: scanblocks asserts MEMBERSHIP
# over a known-true block set, never filter bytes, so a single bulk
# generatetoaddress (one fast RPC) is sufficient and keeps the Core phase short.
log "mining $NBLOCKS empty blocks to $ADDR on Core"
if ! core_cli_retry generatetoaddress "$NBLOCKS" "$ADDR" >/dev/null; then
    kill -0 "$CORE_BG" 2>/dev/null \
        && fail "Core generatetoaddress ($NBLOCKS blocks) failed (oracle alive)" \
        || fail "Core generatetoaddress ($NBLOCKS blocks) failed (oracle DIED — see $CORE_LOG)"
fi
...
# ── 6a. Launch camlcoin NOW (Core chain is built) WITH --blockfilterindex. ─
# Launching here (not before the Core build) keeps camlcoin from sitting idle
# through the chain-build phase, which provokes a watchdog SIGKILL.
CC_OK=0
for attempt in 1 2 3; do
...
PROBE=$(cc_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# ── 6b. Replay ALL of Core's raw blocks into camlcoin via submitblock. ─────
log "replaying Core's $TOTAL raw blocks into camlcoin via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    kill -0 "$CC_PID" 2>/dev/null || fail "camlcoin process died during replay at h=$h (see $CC_LOG)"
    ...
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "camlcoin submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "camlcoin submitblock errored height $h: $sb"
    fi
done
CC_HEIGHT=$(jpy "$(cc_rpc getblockcount '[]')" "d['result']")
[[ "$CC_HEIGHT" == "$TOTAL" ]] || fail "camlcoin height after replay is $CC_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
CC_TIP=$(jpy "$(cc_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$CC_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP caml=$CC_TIP"
log "camlcoin replayed to height $CC_HEIGHT — chains are byte-identical (tip $CC_TIP)"
TIP=$TOTAL
...
for _ in $(seq 1 60); do
    TIPFILT_ERR=$(jpy "$(cc_rpc getblockfilter "[\"$CC_TIP\", \"basic\"]")" "d.get('error',{}).get('code')")
    [[ -z "$TIPFILT_ERR" || "$TIPFILT_ERR" == "None" ]] && break   # no error => filter present
    sleep 1
done

# ── scanblocks helpers ────────────────────────────────────────────────────
# cc_scan <action> <scanobjects-json> <start> <stop>  -> raw JSON response
cc_scan() {
    cc_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (camlcoin)
cc_scan_blocks() {
    jpy "$(cc_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(cc_scan "start" "$NEEDLE" 0 "$TIP")
...
# (Already asserted both lists contain block1 + spend; that IS the projection.)
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(cc_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(cc_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(cc_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(cc_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (camlcoin is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(cc_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (camlcoin is -1)"
# (c) stop < start -> -1.
EE=$(cc_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (camlcoin is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: camlcoin scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
