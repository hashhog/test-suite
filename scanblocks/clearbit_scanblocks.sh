#!/usr/bin/env bash
#
# clearbit_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#   into clearbit via submitblock. After replay both nodes hold the
...
#
# STRICT UNIFORM INTERFACE (mirrors blockfilter/clearbit_getblockfilter.sh):
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS clearbit: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS clearbit: FAIL <short reason>
...
log() { echo "[scanblocks:clearbit] $*" >&2; }
...
pass() {
    echo "SCANBLOCKS clearbit: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS clearbit: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS clearbit: SKIP $*"
    exit 0
}
...
[[ -x "$NODE_BIN" ]]                 || skip "clearbit binary not found at $NODE_BIN (build with: zig build -Doptimize=ReleaseFast)"
...
# ── 4. Launch clearbit on regtest with the basic block filter index. ──────
...
"$NODE_BIN" --regtest --datadir="$CB_DATADIR" --blockfilterindex --metricsport=0 \
...
# the index is off, SKIP (GAP_RE). We pass a probe addr() needle over [0,0].
PROBE=$(cb_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# generatetoaddress can transiently fail on a heavily shared box (multiple
# sibling Core oracles mining at once); retry several times per block, only
# failing hard if the oracle has actually DIED or generation never succeeds.
log "mining $NBLOCKS empty blocks to $ADDR on Core (setmocktime-pinned)"
core_cli setmocktime "$TBASE" >/dev/null 2>&1 || true
gen_one() {  # mine 1 block at deterministic time = TBASE + $1, with retries
    local target_h="$1"
    core_cli setmocktime "$(( TBASE + target_h ))" >/dev/null 2>&1 || true
    local attempt
    for attempt in $(seq 1 8); do
        if core_cli generatetoaddress 1 "$ADDR" >/dev/null 2>&1; then
            return 0
        fi
        kill -0 "$CORE_BG" 2>/dev/null || fail "Core generatetoaddress failed at block $target_h (oracle DIED — see $CORE_LOG)"
        sleep 1
    done
    return 1
}
for (( i=1; i<=NBLOCKS; i++ )); do
    gen_one "$i" || fail "Core generatetoaddress failed at block $i after 8 retries (oracle alive)"
done
...
# output AND the spent prevout). The mocktime was already pinned to TBASE+NBLOCKS+1
# above (so the spend enters the mempool with that time); gen_one re-pins it.
gen_one "$(( NBLOCKS + 1 ))" || fail "Core failed to mine the spend block after retries"
...
# ── 6. Replay ALL of Core's raw blocks into clearbit via submitblock. ─────
log "replaying Core's $TOTAL raw blocks into clearbit via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    ...
    # Defensive: under shared-box load the .cookie can be re-read stale; on an
    # auth failure, refresh the cookie from disk and retry once before failing.
    if echo "$sb" | grep -q "Authentication required"; then
        kill -0 "$CB_PID" 2>/dev/null || { tail -n 20 "$CB_LOG" >&2 2>/dev/null || true; fail "clearbit process died during replay at height $h"; }
        for c in "$CB_DATADIR/.cookie" "$CB_DATADIR/regtest/.cookie"; do
            [[ -f "$c" ]] && CB_COOKIE=$(cat "$c") && break
        done
        sleep 1
    ...
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "clearbit submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "clearbit submitblock errored height $h: $sb"
    fi
done
CB_HEIGHT=$(jpy "$(cb_rpc getblockcount '[]')" "d['result']")
[[ "$CB_HEIGHT" == "$TOTAL" ]] || fail "clearbit height after replay is $CB_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
CB_TIP=$(jpy "$(cb_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$CB_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP cb=$CB_TIP"
log "clearbit replayed to height $CB_HEIGHT — chains are byte-identical (tip $CB_TIP)"
TIP=$TOTAL
...
# ── scanblocks helpers ────────────────────────────────────────────────────
# cb_scan <action> <scanobjects-json> <start> <stop>  -> raw JSON response
cb_scan() {
    cb_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (clearbit)
cb_scan_blocks() {
    jpy "$(cb_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(cb_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(cb_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(cb_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(cb_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(cb_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (clearbit is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(cb_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (clearbit is -1)"
# (c) stop < start -> -1.
EE=$(cb_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (clearbit is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: clearbit scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
