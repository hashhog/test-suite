#!/usr/bin/env bash
#
# haskoin_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#   into haskoin via submitblock. After replay both nodes hold the
...
#
# STRICT UNIFORM INTERFACE (mirrors blockfilter/haskoin_getblockfilter.sh):
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS haskoin: PASS scan=ok shape=ok range=ok errors=ok
...
pass() {
    echo "SCANBLOCKS haskoin: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS haskoin: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS haskoin: SKIP $*"
    exit 0
}
...
# ── 4. Launch haskoin on regtest with the basic block filter index. ───────
...
PROBE=$(hk_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# Belt-and-braces: confirm the basic block filter index is running via getindexinfo.
...
# ── 6. Replay ALL of Core's raw blocks into haskoin via submitblock. ──────
log "replaying Core's $TOTAL raw blocks into haskoin via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    ...
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "haskoin submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "haskoin submitblock errored height $h: $sb"
    fi
done
HK_HEIGHT=$(jpy "$(hk_rpc getblockcount '[]')" "d['result']")
[[ "$HK_HEIGHT" == "$TOTAL" ]] || fail "haskoin height after replay is $HK_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
HK_TIP=$(jpy "$(hk_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$HK_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP hask=$HK_TIP"
log "haskoin replayed to height $HK_HEIGHT — chains are byte-identical (tip $HK_TIP)"
TIP=$TOTAL
...
# ── scanblocks helpers ────────────────────────────────────────────────────
# hk_scan <action> <scanobjects-json> <start> <stop>  -> raw JSON response
hk_scan() {
    hk_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (haskoin)
hk_scan_blocks() {
    jpy "$(hk_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(hk_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(hk_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(hk_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(hk_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(hk_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (haskoin is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(hk_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (haskoin is -1)"
# (c) stop < start -> -1.
EE=$(hk_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (haskoin is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: haskoin scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
