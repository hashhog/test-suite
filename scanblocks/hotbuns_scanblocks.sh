#!/usr/bin/env bash
#
# hotbuns_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#   into hotbuns via submitblock. After replay both nodes hold the
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS hotbuns: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS hotbuns: FAIL <short reason>
#   SKIP: SCANBLOCKS hotbuns: SKIP <no scanblocks RPC | no filter index>
#
# Touches ONLY /tmp/sblk-hotbuns/ + /tmp/sblk-hotbuns-core/ and ports
#   22330/22350 (hotbuns RPC/P2P) + 22332/22352 (Core RPC; P2P unused, -listen=0).
...
HB_DATADIR="/tmp/sblk-hotbuns/$$"
HB_RPC=22330
HB_P2P=22350
HB_LOG="/tmp/sblk-hotbuns-node-$$.log"   # outside the trap-wiped datadir

CORE_DATADIR="/tmp/sblk-hotbuns-core/$$"
CORE_RPC=22332
...
pass() {
    echo "SCANBLOCKS hotbuns: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS hotbuns: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS hotbuns: SKIP $*"
    exit 0
}

# ── 0. Idempotent reset (own ports + own PID scratch only). ───────────────
log "resetting scratch state (pid=$$)"
pkill -f "sblk-hotbuns/$$" 2>/dev/null || true
...
log "launching hotbuns (regtest, --blockfilterindex=1) rpc=:$HB_RPC p2p=:$HB_P2P -> $HB_LOG"
# NOTE: launch via `exec bun src/index.ts` (NOT `bun run …`). Under `exec` the
# subshell is replaced by the node process, so $HB_PID tracks the live node and
# the cleanup trap can kill it. `bun run <script>` forks a child for the script
# and the `run` wrapper exits within ~1-2s, which made $HB_PID look "dead during
# startup" in a race even though the node was up (the getblockfilter harness
# only wins that race by luck). `bun src/index.ts` is the same entrypoint.
(
    cd "$HB_DIR"
...
PROBE=$(hb_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# ── 6. Replay ALL of Core's raw blocks into hotbuns via submitblock. ──────
log "replaying Core's $TOTAL raw blocks into hotbuns via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    ...
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "hotbuns submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "hotbuns submitblock errored height $h: $sb"
    fi
done
HB_HEIGHT=$(jpy "$(hb_rpc getblockcount '[]')" "d['result']")
[[ "$HB_HEIGHT" == "$TOTAL" ]] || fail "hotbuns height after replay is $HB_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
HB_TIP=$(jpy "$(hb_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$HB_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP hotbuns=$HB_TIP"
log "hotbuns replayed to height $HB_HEIGHT — chains are byte-identical (tip $HB_TIP)"
TIP=$TOTAL

# ── scanblocks helpers ────────────────────────────────────────────────────
# hb_scan <action> <scanobjects-json> <start> <stop>  -> raw JSON response
hb_scan() {
    hb_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (hotbuns)
hb_scan_blocks() {
    jpy "$(hb_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(hb_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(hb_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(hb_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(hb_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(hb_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (hotbuns is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(hb_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (hotbuns is -1)"
# (c) stop < start -> -1.
EE=$(hb_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (hotbuns is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: hotbuns scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
