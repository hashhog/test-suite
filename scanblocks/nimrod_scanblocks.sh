#!/usr/bin/env bash
#
# nimrod_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#
# Core ref: bitcoin-core/src/rpc/blockchain.cpp:2531-2716 (scanblocks).
...
#   into nimrod via submitblock. After replay both nodes hold the
...
#
# STRICT UNIFORM INTERFACE (mirrors blockfilter/nimrod_getblockfilter.sh):
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS nimrod: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS nimrod: FAIL <short reason>
...
pass() {
    echo "SCANBLOCKS nimrod: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS nimrod: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS nimrod: SKIP $*"
    exit 0
}
...
[[ -x "$NODE_BIN" ]]                 || skip "nimrod binary not found at $NODE_BIN (build with: nimble build -d:release -y)"
...
PROBE=$(nr_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# Also confirm the index is actually listed (honest SKIP if nimrod ran without it).
...
# Capture Core's scanblocks results + bogus-filtertype/height responses up front
# (the sandbox SIGKILLs bitcoind after a bounded lifetime — read what we need
# while the oracle is alive, before the slow nimrod submitblock import loop).
CORE_RB_FULL=$(core_cli_retry scanblocks start "[\"addr($ADDR)\"]" 0 "$TOTAL" basic \
    | python3 -c "import sys,json; print(' '.join(json.load(sys.stdin)['relevant_blocks']))" 2>/dev/null)
CORE_BOGUS_FT=$(core_cli scanblocks start "[\"addr($ADDR)\"]" 0 "$TOTAL" bogustype 2>&1 | grep -oE '\-5' | head -1)
CORE_BAD_START=$(core_cli scanblocks start "[\"addr($ADDR)\"]" "$(( TOTAL + 1 ))" "$TOTAL" basic 2>&1 | grep -oE '\-1' | head -1)
CORE_BAD_STOP=$(core_cli scanblocks start "[\"addr($ADDR)\"]" 10 5 basic 2>&1 | grep -oE '\-1' | head -1)
log "Core scanblocks (addr->[0,$TOTAL]) relevant_blocks: [$CORE_RB_FULL]"

# ── 6. Replay ALL of Core's raw blocks into nimrod via submitblock. ───────
log "replaying Core's $TOTAL raw blocks into nimrod via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    ...
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "nimrod submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "nimrod submitblock errored height $h: $sb"
    fi
done
NR_HEIGHT=$(jpy "$(nr_rpc getblockcount '[]')" "d['result']")
[[ "$NR_HEIGHT" == "$TOTAL" ]] || fail "nimrod height after replay is $NR_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
NR_TIP=$(jpy "$(nr_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$NR_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP nimrod=$NR_TIP"
log "nimrod replayed to height $NR_HEIGHT — chains are byte-identical (tip $NR_TIP)"
TIP=$TOTAL
...
# ── scanblocks helpers ────────────────────────────────────────────────────
# nr_scan <action> <scanobjects-json> <start> <stop>  -> raw JSON response
nr_scan() {
    nr_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (nimrod)
nr_scan_blocks() {
    jpy "$(nr_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
in_list() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(nr_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
[[ -n "$CORE_RB_FULL" ]] || fail "Core scanblocks returned empty/failed; CORE_RB_FULL='$CORE_RB_FULL'"
# Core MUST also report both known-true blocks (proves the oracle agrees).
in_list "$BLOCK1_HASH" "$CORE_RB_FULL"     || fail "Core scanblocks missing block1 (oracle sanity); core=[$CORE_RB_FULL]"
in_list "$SPEND_BLOCKHASH" "$CORE_RB_FULL" || fail "Core scanblocks missing spend block (oracle sanity); core=[$CORE_RB_FULL]"
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(nr_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(nr_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(nr_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(nr_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CORE_BOGUS_FT" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (nimrod is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(nr_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CORE_BAD_START" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (nimrod is -1)"
# (c) stop < start -> -1.
EE=$(nr_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CORE_BAD_STOP" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (nimrod is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: nimrod scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
