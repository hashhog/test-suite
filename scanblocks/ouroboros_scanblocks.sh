#!/usr/bin/env bash
#
# ouroboros_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#   into ouroboros via submitblock. After replay both nodes hold the
...
#
# WHAT MUST HOLD (per the strict fan-out contract; mirrors rustoshi_scanblocks):
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS ouroboros: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS ouroboros: FAIL <short reason>
#   SKIP: SCANBLOCKS ouroboros: SKIP <no scanblocks RPC | no filter index>
#
# Touches ONLY /tmp/sblk-ouroboros/ + /tmp/sblk-core/ and ports
#   22330/22350 (ouroboros RPC/P2P) + 22332/22352 (Core RPC; P2P unused, -listen=0).
...
log() { echo "[scanblocks:ouroboros] $*" >&2; }
...
cleanup() {
    ...
    pkill -f "sblk-ouroboros/$$" 2>/dev/null || true
...
pass() {
    echo "SCANBLOCKS ouroboros: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS ouroboros: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS ouroboros: SKIP $*"
    exit 0
}
...
[[ -f "$OURO_DIR/src/ouroboros/cli.py" ]] || skip "ouroboros cli.py not found under $OURO_DIR"
...
# ── 4. Launch ouroboros on regtest with the basic block filter index. ─────
log "launching ouroboros (regtest, --blockfilterindex) rpc=:$OU_RPC p2p=:$OU_P2P -> $OU_LOG"
(
    cd "$OURO_DIR"
...
echo "$(ou_rpc getblockcount '[]')" | grep -q '"result"' || fail "ouroboros RPC never responded within 150s"
...
PROBE=$(ou_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
# ── 6. Replay ALL of Core's raw blocks into ouroboros via submitblock. ─────
log "replaying Core's $TOTAL raw blocks into ouroboros via submitblock"
for (( h=1; h<=TOTAL; h++ )); do
    ...
    echo "$sb" | grep -q '"result"'           || fail "ouroboros submitblock errored at height $h: $sb"
    sbres=$(jpy "$sb" "d.get('result')")
    sberr=$(jpy "$sb" "d.get('error')")
    if [[ -n "$sbres" && "$sbres" != "None" && "$sbres" != "duplicate" && "$sbres" != "inconclusive" ]]; then
        fail "ouroboros submitblock rejected height $h: result='$sbres' raw_resp=$sb"
    fi
    if [[ -n "$sberr" && "$sberr" != "None" ]]; then
        fail "ouroboros submitblock errored height $h: $sb"
    fi
done
OU_HEIGHT=$(jpy "$(ou_rpc getblockcount '[]')" "d['result']")
[[ "$OU_HEIGHT" == "$TOTAL" ]] || fail "ouroboros height after replay is $OU_HEIGHT, expected $TOTAL"
CORE_TIP=$(core_cli_retry getbestblockhash)
OU_TIP=$(jpy "$(ou_rpc getbestblockhash '[]')" "d['result']")
[[ "$CORE_TIP" == "$OU_TIP" ]] || fail "tip mismatch after replay: core=$CORE_TIP ouro=$OU_TIP"
log "ouroboros replayed to height $OU_HEIGHT — chains are byte-identical (tip $OU_TIP)"
TIP=$TOTAL

# Give the Python filter index a moment to finish its off-thread add_block for
# the last connected blocks (block-connect hook is best-effort/async).
sleep 2

# ── scanblocks helpers ────────────────────────────────────────────────────
ou_scan() {
    # args: action scanobjects start stop  -> raw JSON response
    ou_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
ou_scan_blocks() {
    jpy "$(ou_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(ou_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(ou_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(ou_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(ou_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(ou_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (ouroboros is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(ou_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (ouroboros is -1)"
# (c) stop < start -> -1.
EE=$(ou_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (ouroboros is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: ouroboros scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
