#!/usr/bin/env bash
#
# rustoshi_scanblocks.sh — self-contained scanblocks Core-parity test.
#
...
#   into rustoshi via submitblock. After replay both nodes hold the
...
#
# STRICT UNIFORM INTERFACE (mirrors blockfilter/rustoshi_getblockfilter.sh):
...
#
# Summary line (stdout):
#   PASS: SCANBLOCKS rustoshi: PASS scan=ok shape=ok range=ok errors=ok
#   FAIL: SCANBLOCKS rustoshi: FAIL <short reason>
#   SKIP: SCANBLOCKS rustoshi: SKIP <no scanblocks RPC | no filter index>
#
# Touches ONLY /tmp/sblk-rustoshi/ + /tmp/sblk-core/ and ports
#   22230/22250 (rustoshi RPC/P2P) + 22232/22252 (Core RPC; P2P unused, -listen=0).
...
CORE_RPC=22232
CORE_P2P=22252   # declared but Core launched -listen=0 (no P2P listener)
...
log() { echo "[scanblocks:rustoshi] $*" >&2; }
...
pass() {
    echo "SCANBLOCKS rustoshi: PASS scan=$1 shape=$2 range=$3 errors=$4"
    exit 0
}
fail() {
    echo "SCANBLOCKS rustoshi: FAIL $*"
    exit 1
}
skip() {
    echo "SCANBLOCKS rustoshi: SKIP $*"
    exit 0
}

# ── 0. Idempotent reset (own ports + own PID scratch only). ───────────────
log "resetting scratch state (pid=$$)"
pkill -f "sblk-rustoshi/$$" 2>/dev/null || true
...
PROBE=$(rs_rpc scanblocks "[\"start\", [\"addr($ADDR)\"], 0, 0, \"basic\"]")
...
log "rustoshi replayed to height $RS_HEIGHT — chains are byte-identical (tip $RS_TIP)"
TIP=$TOTAL

# ── scanblocks helpers ────────────────────────────────────────────────────
# rs_scan_field <action> <scanobjects-json> <start> <stop> <field>
rs_scan() {
    # args: action scanobjects start stop  -> raw JSON response
    rs_rpc scanblocks "[\"$1\", $2, $3, $4, \"basic\"]"
}
# return space-separated relevant_blocks list (rustoshi)
rs_scan_blocks() {
    jpy "$(rs_scan "$1" "$2" "$3" "$4")" "' '.join(d['result']['relevant_blocks'])"
}
co_scan_blocks() {
...
NEEDLE="[\"addr($ADDR)\"]"
RESP=$(rs_scan "start" "$NEEDLE" 0 "$TIP")
...
# ════════════════════════════════════════════════════════════════════════
NEG_RB=$(rs_scan_blocks "start" "[\"addr($NEGADDR)\"]" 0 "$TIP")
...
W1=$(rs_scan "start" "$NEEDLE" "$SPEND_HEIGHT" "$SPEND_HEIGHT")
...
W2_RB=$(rs_scan_blocks "start" "$NEEDLE" 2 50)
...
EF=$(rs_rpc scanblocks "[\"start\", $NEEDLE, 0, $TIP, \"bogustype\"]")
...
[[ "$CEF" == "-5" ]] || log "WARNING: could not confirm Core returns -5 for unknown filtertype (rustoshi is -5)"
# (b) out-of-range start (tip+1) -> -1.
ES=$(rs_rpc scanblocks "[\"start\", $NEEDLE, $(( TIP + 1 )), $TIP, \"basic\"]")
...
[[ "$CES" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for out-of-range start (rustoshi is -1)"
# (c) stop < start -> -1.
EE=$(rs_rpc scanblocks "[\"start\", $NEEDLE, 10, 5, \"basic\"]")
...
[[ "$CEE" == "-1" ]] || log "WARNING: could not confirm Core returns -1 for stop<start (rustoshi is -1)"
ERR_T="ok"
log "errors ok: filtertype=-5 start>tip=-1 stop<start=-1"

log "PASS: rustoshi scanblocks matches Core on membership + core-superset + negative + range + actions + errors"
