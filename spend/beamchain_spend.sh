#!/usr/bin/env bash
#
# beamchain_spend.sh — self-contained wallet-native spend regression test.
#
# Codifies the "wallet can see and spend its own coins" cell — the successor
# to the 10/10 wallet-recovery cell (beamchain_recovery.sh). Proves the full
...
# loaded wallet's getbalance is an honest sender-side measurement (no second
# wallet to pollute the shared wallet-UTXO view).
#
# STRICT UNIFORM INTERFACE (mirrors beamchain_recovery.sh exactly): no required
# args, idempotent, trap cleanup, scratch datadir + unique ports, single clean
# summary line on stdout. All noise -> stderr / logfile.
#
# Summary line (stdout):
#   PASS: SPEND beamchain: PASS funded=<X> sent=<Y> recipient=<Y> sender_debited=<Y+fee> ...
#   FAIL: SPEND beamchain: FAIL <short reason>
#
# Touches ONLY /tmp/spendref-beamchain/ and ports 39700 (RPC) / 39720 (P2P).
...
DATADIR="/tmp/spendref-beamchain"
RPC_PORT=39700
P2P_PORT=39720
...
cleanup() {
    ...
    # Defensive: free the ports in case a child/escript lingers.
...
pass() {
    echo "SPEND beamchain: PASS funded=$1 sent=$2 recipient=$3 sender_debited=$4 fee_sats=$5 mempool=clean"
    exit 0
}
fail() {
    echo "SPEND beamchain: FAIL $*"
    exit 1
}
...
-sname beamchain_spendref_$$
-setcookie beamchain_spendref
+P 1048576
+K true
+A 64
ERLVM
...
# the difference (50 BTC matured in - debited out) must equal the change/credit
# arithmetic. Concretely, the amount the wallet LOST to the spend is:
#   debited = (mature coins consumed) - (change returned)
# We verify the on-chain transfer (recipient got exactly SEND_SATS) AND that the
# wallet's spendable balance decreased by at least SEND_SATS net of the single
# 50 BTC maturation (i.e. accounting drift is only the fee, a few hundred sats).
...
# Fee must be positive (a real fee was paid) and small (regtest 5 sat/vB on a
