# _sign_lib.sh — shared driver for walletdiff SLICE 3 (signing + sighash,
# incl. taproot). Sourced, not run. Reuses the launch/cleanup/log plumbing from
# _lib.sh (launch_core, wd_cleanup, pass/fail/skip/blocked_line, CORE_BIN/CLI)
# and adds the signing driver. ALL comparison logic lives in probe_sign.py; the
# per-impl adapter supplies ONLY launch/RPC plumbing (harness-script-consistency
# memory) and may not weaken the comparison.
#
# An adapter (<impl>_walletsign.sh) must, before calling walletdiff_signing_main:
...
#   - set IMPL_DATADIR CORE_DATADIR (scratch under /tmp/walletsign-*)
...
#   WALLETDIFF <impl>: PASS sign=2/2 accept=2/2 [p2wpkh=ACCEPT p2tr=ACCEPT]
#   WALLETDIFF <impl>: FAIL sign=.. accept=.. [p2wpkh=ACCEPT p2tr=NO-RECEIVE]
...
walletdiff_signing_main() {
    ...
    [[ -s "$SIGN_PROBE" ]] || fail_line "shared comparator missing: $SIGN_PROBE"
    ...
    local results_json="$RESULTS_DIR/walletsign-$IMPL-$ts.json"
    python3 "$SIGN_PROBE" --impl "$IMPL" \
