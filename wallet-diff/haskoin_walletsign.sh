#!/usr/bin/env bash
#
# haskoin_walletsign.sh — walletdiff SLICE 3 (signing + sighash, incl. taproot,
# P2.1) for haskoin (Haskell), differential against a REAL bitcoind regtest
# oracle.
#
# Plumbing ONLY (harness-script-consistency memory): all comparison logic lives
# in wallet-diff/probe_sign.py. This script just launches haskoin + the Core
...
#
# Interface: no args; idempotent; scratch /tmp/walletsign-haskoin* only;
...
IMPL_DATADIR="/tmp/walletsign-haskoin"
CORE_DATADIR="/tmp/walletsign-haskoin-core"
