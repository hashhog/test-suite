#!/usr/bin/env bash
#
# nimrod_watchonly.sh — Core-faithful WATCH-ONLY import differential test.
#
...
# (maturity; tip 104) BEFORE any import; createwallet wo (dpk, blank);
# importdescriptors addr(A)#chk + wpkh(PUBW)#chk ts=0; negatives (-5 no
# checksum / -4 privkey); addr(B)#chk ts=0 must credit the PRE-IMPORT
# funding; observability (listunspent / balance / getaddressinfo ismine);
...
BIN="$BASEDIR/nimrod/bin/nimrod"
DATADIR="/tmp/hashhog-wofleet-nimrod"
RPC_PORT=21401
P2P_PORT=21421
...
fail() { echo "WATCHONLY nimrod: FAIL $*"; exit 1; }
boot_gap() { echo "WATCHONLY nimrod: FAIL cannot boot: $*"; exit 1; }
...
# ── RPC helper (cookie auth; optional /wallet/<name> path — nimrod shape). ──
rpc() {
    local method="$1" params="${2:-[]}" wallet="${3:-}" auth="" path="/"
    [[ -f "$COOKIE_FILE" ]] && auth="-u $(cat "$COOKIE_FILE")"
    [[ -n "$wallet" ]] && path="/wallet/$wallet"
    # shellcheck disable=SC2086
    curl -s --max-time 40 $auth -H 'content-type: text/plain' \
...
# ── 1. Preconditions. ──────────────────────────────────────────────────────
[[ -x "$BIN" ]] || boot_gap "nimrod binary not found at $BIN"
...
# ── 3. Launch nimrod on regtest ('start' subcommand). ──────────────────────
log "launching nimrod regtest -> $LOG"
...
while (( $(date +%s) < deadline )); do
    if [[ -f "$COOKIE_FILE" ]] && rpc getblockcount | grep -q '"result"'; then up=1; break; fi
...
echo "$r" | grep -q '"result"' || fail "createwallet w1 failed: $(echo "$r" | head -c 160)"
for spec in "1 $A_ADDR" "1 $B_ADDR" "1 $W_ADDR" "101 $M_ADDR"; do
    n=${spec%% *}; ad=${spec#* }
    r=$(rpc generatetoaddress "[$n, \"$ad\"]" "w1")
    echo "$r" | grep -q '"result"' || fail "generatetoaddress $n->$ad failed: $(echo "$r" | head -c 160)"
done
...
OUT=$(python3 "$WO_LIB" check --impl nimrod \
